...
extern ServoEasing Servo5; // Conectar con la instancia principal

void ServoControlling::setupGripper(int p1, int p2, int maxA) {
    // Asignamos el Gripper al PIN 4 del PCA9685
    Servo5.attach(4, 0, p1, p2); 
    Servo5.setEasingType(EASE_QUADRATIC_IN_OUT);
}

void ServoControlling::setGripperAngle(float angle) {
    // Fija el target. El movimiento físico ocurre sincronizado en el loop interno.
    Servo5.setEaseTo(angle); 
}
