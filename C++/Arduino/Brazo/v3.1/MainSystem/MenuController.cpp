...
extern ServoEasing Servo5; // Asegura el acceso a la instancia global del 5° servo (Gripper)
...
void MenuController::update() {
    ...
    // Variables estáticas para mantener la posición del gripper y controlar el flujo
    static float targetQ5 = 90.0f; 
    bool updateGripper = false;

    if (key != '\0') {
        switch(key) {
            case 'A': 
                key = '#'; // Botón A -> Funciona como Atrás / Salir
                break; 
            case 'B': 
                targetQ5 += 5.0f; // Botón B -> Abrir gradualmente (ajusta el paso si es necesario)
                updateGripper = true;
                break;
            case 'C':
                targetQ5 -= 5.0f; // Botón C -> Cerrar gradualmente
                updateGripper = true;
                break;
            case 'D':
    ...
    // PROTECCIÓN MECÁNICA: Solo se envía el comando I2C si se pulsó B o C
    if (updateGripper) {
        Servo5.write(targetQ5);
    }

    // Ejecuta el estado actual (la pulsación de B o C pasará de largo sin afectar los menús)
    if (ptrState != nullptr) {
...
void MenuController::handleJoyAngular(char key, int joy) {
    ...
    if (isTouchingStick) {
        ...
        if (millis() - lastStepTime > (unsigned long)dt_ms) {
            ...
            // Guardamos el estado anterior para la derivada de velocidad
            float oldQ1 = targetQ1, oldQ2 = targetQ2, oldQ3 = targetQ3, oldQ4 = targetQ4;

            // 3. SUMA MATEMÁTICA SOBRE TARGETS
            ...
            // CÁLCULO DE VELOCIDAD ANGULAR (Omega = Delta_Theta / Delta_Tiempo)
            float dt_seg = dt_ms / 1000.0f;
            v1 = (targetQ1 - oldQ1) / dt_seg;
            v2 = (targetQ2 - oldQ2) / dt_seg;
            v3 = (targetQ3 - oldQ3) / dt_seg;
            v4 = _servo->isServo4 ? ((targetQ4 - oldQ4) / dt_seg) : 0.0f;

            // 4. USO DE FABS() EN LUGAR DE ABS()
            ...
            int dynamicSpeed = (int)((max_dTheta / dt_seg) * 1.3f); 
            if (dynamicSpeed < 8) dynamicSpeed = 8; 
            if (dynamicSpeed > 150) dynamicSpeed = 150; 
        ...
        v1 = v2 = v3 = v4 = 0.0f; // Estático -> omega = 0
    }
    ...
    if (millis() - lastUIDraw > 250) {
        ...
        UI->drawMoveMenu(0, pos.x(), pos.y(), pos.z(), _servo->getAngle(1), _servo->getAngle(2), _servo->getAngle(3), _servo->getAngle(4), v1, v2, v3, v4, currentMult, true, _servo->isServo4);
    }
}
...
void MenuController::handleJoyPosition(char key, int joy) {
    ...
    // El pulsador del Joystick 1 cambia el multiplicador de velocidad
    static unsigned long lastSwPress1 = 0;
    if (_sys->getJoySwState(1) && (millis() - lastSwPress1 > 300)) {
        multiplierIndex = (multiplierIndex + 1) % 3; lastSwPress1 = millis();
    }

    // El pulsador del Joystick 2 cambia el modo PHI
    static unsigned long lastSwPress2 = 0;
    if (_sys->getJoySwState(2) && (millis() - lastSwPress2 > 300)) {
    ...
    // Zona muerta estricta para evitar vibraciones
    auto getStep = [](int val) -> int {
        if (val > 3800) return 2; else if (val > 3000) return 1;
        else if (val < 200) return -2; else if (val < 1000) return -1;
        return 0;
    ...
    if (isTouchingStick) {
        static unsigned long lastStepTime = 0;
        float dt_segundos = 0.06f; // Ciclo de 60ms
        ...
        if (millis() - lastStepTime > 60) {
            ...
            if (testIk) {
                ...
                // CÁLCULO DE VELOCIDAD ANGULAR DESDE LA CINEMÁTICA INVERSA
                v1 = (t1 - _servo->getAngle(1)) / dt_segundos;
                v2 = (t2 - _servo->getAngle(2)) / dt_segundos;
                v3 = (t3 - _servo->getAngle(3)) / dt_segundos;
                v4 = _servo->isServo4 ? ((t4 - _servo->getAngle(4)) / dt_segundos) : 0.0f;
                ...
                if (dynamicSpeed < 15) dynamicSpeed = 15; 

                _servo->ReachForAnglesContinuous(t1, t2, t3, t4, dynamicSpeed);
            } else {
                ikPossible = false;
                v1 = v2 = v3 = v4 = 0.0f; // Cinematica bloqueada -> velocidad cero
            }
        }
        wasMoving = true;
    } 
    ...
    if (millis() - lastUIDraw > 250) {
        lastUIDraw = millis();
        V3 pos = _servo->getPos(); 
        UI->drawMoveMenu(1, pos.x(), pos.y(), pos.z(), _servo->getAngle(1), _servo->getAngle(2), _servo->getAngle(3), _servo->getAngle(4), v1, v2, v3, v4, currentMult, ikPossible, _servo->isServo4);
    }
}
// --- HANDLERS DE TOOLS ) ---
