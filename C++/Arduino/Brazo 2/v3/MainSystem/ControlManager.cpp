...
void ControlManager::applyVirtualWalls(float dt) {
    ...
    // 1. Revisamos si ALGÚN motor ha tocado la frontera e intenta seguir avanzando
    for (int i = 0; i < 4; i++) {
        if ((sensor_q[i] >= (_max_angles[i] - margin) && joints[i].q_dot_ref > 0.0f) ||
            (sensor_q[i] <= (_min_angles[i] + margin) && joints[i].q_dot_ref < 0.0f)) {
            limit_hit = true;
            break; // Encontramos infracción, no necesitamos seguir buscando
        }
    }
    // 2. Si hubo colisión, abortamos TODO de inmediato
    if (limit_hit) {
        stopRobot(); // <-- Esto pone traj_space = 'S', apaga la interpolación y pone a 0 todas las velocidades
...
bool ControlManager::AngleLoop(float dt) {
    ...
    for(int i = 0; i < 4; i++) {
        ...
        // =====================================================
        // 1. HISTÉRESIS: Zonas separadas de Apagado y Encendido
        // =====================================================
        ...
        if (!enable_interpolation && std::abs(error_pos) < active_tolerance) {
            ...
            // =====================================================
            // 2. EMPUJÓN ESPECÍFICO POR MOTOR
            // =====================================================
            // M1(Base), M2(Hombro), M3(Codo), M4(Muñeca)
...
bool ControlManager::VelocityLoop(float dt) {
    ...
    for(int i = 0; i < 4; i++) {
        ...
        // ============================================================
        // 1. PREALIMENTACIÓN (FEEDFORWARD) - ¡LA PIEZA QUE FALTA!
        // ============================================================
        // Recuperamos el Kv físico real del motor para inyectar PWM instantáneo
        float tau_mec = 0.15f; 
        float tau_c = 0.80f; 
        ...
        // 2. PID TRADICIONAL (Ahora solo corrige micro-errores ambientales)
        float pwm_pid = (joints[i].Kp_vel * error_vel) + (joints[i].Ki_vel * int_vel_val);
        
        // El PWM crudo ahora es predictivo + correctivo
        ...
        // ============================================================
        // CORTE DEFINITIVO (Elimina vibración en estático)
        // ============================================================
...
bool ControlManager::movA(float q1, float q2, float q3, float q4, float speed_deg_s) {
    ...
    // 1. PREDICCIÓN DE LÍMITES
    for(int i = 0; i < 4; i++) { 
        if (target_rad[i] < _min_angles[i] || target_rad[i] > _max_angles[i]) {
            Serial.println("\n[-] Error: MoveA abortado. Peligro de colisión mecánica.");
            Serial.printf("    Angulos objetivo: M1=%.1f°, M2=%.1f°, M3=%.1f°, M4=%.1f°\n", 
                          target_rad[0] * 180.0f/M_PI, target_rad[1] * 180.0f/M_PI, 
                          target_rad[2] * 180.0f/M_PI, target_rad[3] * 180.0f/M_PI);
            Serial.printf("    Fallo en M%d: Objetivo (%.1f°) fuera del rango permitido [%.1f° a %.1f°]\n", 
                          i+1, target_rad[i] * 180.0f/M_PI, 
                          _min_angles[i] * 180.0f/M_PI, _max_angles[i] * 180.0f/M_PI);
            return false;
        }
    }
    ...
    float max_diff = 0.0f;
    for(int i = 0; i < 4; i++) { 
        start_Q[i] = sensor_q[i]; 
        final_target_Q[i] = target_rad[i];
    ...
    speed_rad_s = constrain(speed_rad_s, 0.05f, _max_vel_rad); 
...
bool ControlManager::movLJc(float x, float y, float z, float speed_mm_s) {
    ...
    if (!_kinematic->pos2Angle(x, y, z, th1_deg, th2_deg, th3_deg)) {
        Serial.println("\n[-] Error: MoveL abortado. Posición matemáticamente inalcanzable.");
        return false;
    }
    
    float target_rad[3] = { th1_deg * (float)M_PI/180.0f, th2_deg * (float)M_PI/180.0f, th3_deg * (float)M_PI/180.0f };
    
    // 2. PREDICCIÓN FÍSICA (Límites mecánicos)
    for(int i=0; i<3; i++) {
        if(target_rad[i] < _min_angles[i] || target_rad[i] > _max_angles[i]) {
            Serial.println("\n[-] Error: MoveL abortado. Peligro de colisión mecánica.");
            Serial.printf("    Angulos calculados IK: M1=%.1f°, M2=%.1f°, M3=%.1f°\n", 
...
bool ControlManager::movLJc(float x, float y, float z, float speed_mm_s, float phi_rad) {
    ...
    // 1. CORRECCIÓN: Convertir los radianes de entrada a grados para la predicción
    float phi_deg = phi_rad * 180.0f / M_PI;
    
    // 2. PREDICCIÓN MATEMÁTICA (Usamos phi_deg)
    if (!_kinematic->pos2Angle(x, y, z, th1_deg, th2_deg, th3_deg, th4_deg, phi_deg)) {
        Serial.println("\n[-] Error: MoveL (Phi) abortado. Posición matemáticamente inalcanzable.");
        return false;
    }
    
    float target_rad[4] = { th1_deg * (float)M_PI/180.0f, th2_deg * (float)M_PI/180.0f, th3_deg * (float)M_PI/180.0f, th4_deg * (float)M_PI/180.0f };
    
    // 3. PREDICCIÓN FÍSICA (Límites mecánicos)
    for(int i=0; i<4; i++) {
        if(target_rad[i] < _min_angles[i] || target_rad[i] > _max_angles[i]) {
            Serial.println("\n[-] Error: MoveL (Phi) abortado. Peligro de colisión mecánica.");
            Serial.printf("    Angulos calculados IK: M1=%.1f°, M2=%.1f°, M3=%.1f°, M4=%.1f°\n", 
                          th1_deg, th2_deg, th3_deg, th4_deg);
            Serial.printf("    Fallo en M%d: Objetivo (%.1f°) fuera del rango permitido [%.1f° a %.1f°]\n", 
                          i+1, target_rad[i] * 180.0f/M_PI, 
                          _min_angles[i] * 180.0f/M_PI, _max_angles[i] * 180.0f/M_PI);
            return false;
    ...
    // CORRECCIÓN: phi_rad ya viene en radianes, asignamos directo sin conversiones destructivas
    final_target_Phi = phi_rad; 
...
bool ControlManager::movJ(float x, float y, float z, float speed_mm_s, bool elbowUp) {
    ...
    // 1. PREDICCIÓN MATEMÁTICA
    if (!_kinematic->pos2Angle(x, y, z, th1_deg, th2_deg, th3_deg, 0.0f, elbowUp)) {
        Serial.println("\n[-] Error: MoveJ abortado. Posición matemáticamente inalcanzable.");
        return false;
    }
    ...
    for(int i=0; i<4; i++) {
        if(target_rad[i] < _min_angles[i] || target_rad[i] > _max_angles[i]) {
            Serial.println("\n[-] Error: MoveJ abortado. Peligro de colisión mecánica.");
            Serial.printf("    Angulos calculados IK: M1=%.1f°, M2=%.1f°, M3=%.1f°, M4=0.0°\n", 
                          th1_deg, th2_deg, th3_deg);
            Serial.printf("    Fallo en M%d: Objetivo (%.1f°) fuera del rango permitido [%.1f° a %.1f°]\n", 
                          i+1, target_rad[i] * 180.0f/M_PI, 
                          _min_angles[i] * 180.0f/M_PI, _max_angles[i] * 180.0f/M_PI);
...
bool ControlManager::movJ(float x, float y, float z, float speed_mm_s, float phi_val, bool elbowUp) {
    ...
    // 1. PREDICCIÓN MATEMÁTICA
    if (!_kinematic->pos2Angle(x, y, z, th1_deg, th2_deg, th3_deg, th4_deg, phi_deg, elbowUp)) {
        Serial.println("\n[-] Error: MoveJ (Phi) abortado. Posición matemáticamente inalcanzable.");
        return false;
    }
    ...
    for(int i=0; i<4; i++) {
        if(target_rad[i] < _min_angles[i] || target_rad[i] > _max_angles[i]) {
            Serial.println("\n[-] Error: MoveJ (Phi) abortado. Peligro de colisión mecánica.");
            Serial.printf("    Angulos calculados IK: M1=%.1f°, M2=%.1f°, M3=%.1f°, M4=%.1f°\n", 
                          th1_deg, th2_deg, th3_deg, th4_deg);
            Serial.printf("    Fallo en M%d: Objetivo (%.1f°) fuera del rango permitido [%.1f° a %.1f°]\n", 
                          i+1, target_rad[i] * 180.0f/M_PI, 
