...
class SobolAnalysis:
    ...
    def analyze(self, Y):
        return sobol_analyzer.analyze(self.problem, Y, calc_second_order=True)


def plot_sobol_indices(names, S1, ST, titulo="Índices de Sobol"):
    x = np.arange(len(names))
    width = 0.35

    fig, ax = plt.subplots(figsize=(12, 6))
    ax.bar(x - width/2, S1, width, label='S1 (Primer orden)')
...
if __name__ == "__main__":
    distribuciones = {
        "Vlect": {"tipo": "normal", "params": {"media": 0.0003, "desv": 1}},
        ...
        "L":     {"tipo": "normal", "params": {"media": 0.1, "desv": 0.02}},
        "x":     {"tipo": "normal", "params": {"media": 0.05, "desv": 0.01}},
        ...
        "lg":    {"tipo": "uniform", "params": {"min": 0.05, "max": 0.051}},
        "K":     {"tipo": "uniform", "params": {"min": 101, "max": 105}}
    }

    print("Generando muestras y ejecutando modelo...")
    sobol = SobolAnalysis(distribuciones, n=2**12)
    sobol.calculate()
    Y = sobol.run_model()
