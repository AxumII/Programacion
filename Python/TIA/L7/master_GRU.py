...
class MasterGRU(nn.Module):
    ...
    def fit(self, train_loader, val_loader, optimizer, criterion, device, verbose=True):
        ...
        if verbose:
            print(f"▶ Iniciando entrenamiento GRU por {self.epochs} épocas...")
        ...
        if verbose:
            print(f"✔ Entrenamiento GRU Finalizado en: {history['time']:.2f}s")
        return history

    def evaluate(self, history, test_loader, device, exp_name="MasterGRU", verbose=True):
