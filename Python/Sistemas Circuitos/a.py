...
def A():
    def analisis(Av=None, B=None, Rl=None, Rc=None, Icq=None, Rlp=None, Vcc=None):
        ...
        ########### Análisis DC #############################
        # 1. Cálculo de Rth
        Rth = 1 / (1 / Rc + 1 / Rlp)  # Resistencia de Thevenin equivalente

        # 2. Cálculo del nodo de voltaje Vb
        Vb = Icq * Rth + Vbe

        # 3. Divisor de tensión para hallar Vnodo
        Vnodo = Vb  # Vnodo = Vb debido a la caída en la resistencia Rth

        # 4. Cálculo de R1 y R2 a partir de Vcc y Vnodo
        R1 = (Vcc * Rth) / Vnodo
        R2 = Rth / ((1 - (Vnodo / Vcc)))

        # 5. Rac, resistencia AC equivalente
        Rac = (1 / (1 / Rc + 1 / Rlp)) + Re

        # 6. Rdc, resistencia DC
        Rdc = Rc + Re
        ...
        Vcc_eq = sp.Eq(Vcc, Rc * Ic_sym + Vce + Re * Ic_sym )  #Ic_sym * (1 + (1 / B))
...
analisis = A()
resultados = analisis(Av=50, B=B, Rl=Rl, Rc=500, Icq=0.001, Rlp=Rlp_valor, Vcc=Vcc)
